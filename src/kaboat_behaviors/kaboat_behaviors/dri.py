...
AVOIDANCE.MD §3-1 의 구현. ROS 의존 없음 — `build_dri` 는
...
위험도 정의):
  ...
  셀 결합은 **max** — 합산(Σ)이 아니다 (§3-1). 충돌 안전은 **최근접 장애물**이
  ...
  max 는 DRI 를 [0, max A] 로 **유계**로 만들어 §3-2 비용 균형도 안정시킨다.
...
미관측(-1) 셀은 기여 0 = free 취급) — 위험 취급하면 센서 범위 밖으로 영영
...
class DriParams:
    ...
    # 히트 횟수의 함수다(1회→70, 2회→85, 3회→93). 90 = "3히트는 봐야 믿는다"를 전
    # 격자에 적용 — 근거리 자세흔들림 유령(1~2히트)을 회피 단에서 통째로 배제한다.
    # 대가로 원거리 성긴 부표(셀당 1히트=70)도 투명해지지만, 장애물 회피 단에서는
    # 먼 1히트 부표 조기감지를 포기해도 된다고 결정(가까이서 반응해도 충분).
    # ⚠️ 이 전역 90 문턱이 occupancy_grid 의 8m confirm 게이트(근거리 93 요구)를
    # 대체하므로 그 게이트는 중복 → 제거 가능. (예전엔 50 = far 1히트도 통과였다.)
    occ_threshold: int = 90
...
def build_dri(grid, boat_xy, boat_yaw: float, p: DriParams) -> DriGrid:
    """OccupancyGrid msg → DriGrid (§3-1).

    grid 는 nav_msgs/OccupancyGrid 와 같은 필드(info.resolution/width/height,
    info.origin.position, data)를 가진 객체면 무엇이든 된다.
    """
    ...
    # unknown(-1) 은 occ_threshold 비교에서 자연히 탈락 = free 취급)
    ...
    for k in range(oy.size):
        ...
        # max 결합 — 겹치는 셀은 더하지 않고 더 위험한 쪽을 남긴다 (§3-1)
