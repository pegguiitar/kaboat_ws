...
class DriParams:
    ...
    # 히트 횟수의 함수라(1회→70, 2회→85, 3회→93) 이 값이 곧 "몇 번 봐야 믿을까" 다.
    # 50 이면 단발 노이즈도 즉시 장애물이 된다 — 75 = 2회 요구 (§7-3).
    occ_threshold: int = 75
    sigma: float = 1.0          # 등방 σ 기준값(스케일 전) [m] — 곧 berth 폭
    ...
    a_head: float = 1.0         # 진폭 A 전방성 확대 게인
    # k_*/a_* 는 작업 5 에서 sim 을 보며 정할 값이라 위 숫자는 출발점일 뿐이다.
    # 넷 다 0 이면 크기·세기가 고정된 균일 가우시안으로 축약된다(디버그 기준선).
