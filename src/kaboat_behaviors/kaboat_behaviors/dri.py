...
class DriParams:
    ...
    # 히트 횟수의 함수다(1회→70, 2회→85, 3회→93, 4회 이상 포화→97). 95 = "4히트는
    # 봐야 믿는다"를 전 격자에 적용. 파도로 tilt 가 오래 지속되면 해수면 셀이 3히트(93)
    # 까지 쌓여 90 문턱을 넘겨 유령 장애물이 됐다 — 부표는 근거리에서 더 쌓여 포화(97)
    # 하므로, 95 로 올리면 3히트 해수면 유령만 잘리고 부표는 남는다.
    # 대가로 원거리 성긴 부표(1~3히트)도 투명해지지만, 회피 단에서는 가까이서 반응해도
    # 충분하다고 결정. (occupancy_grid 의 옛 8m confirm 게이트는 이 전역 문턱으로
    # 대체되어 제거됨. 예전엔 50 = far 1히트도 통과였다.)
    occ_threshold: int = 95
    sigma: float = 0.53         # 등방 σ **상한**(코앞·정면일 때) [m] — 곧 berth 폭.
