...
skeleton 구현 — 아직 YOLO 아님:
  ...
  shape 은 "unknown", distance 는 -1.0 으로 발행 (인터페이스만 유효).

TODO(팀): YOLOv8 추론으로 교체 —
  1) 모양 3클래스(circle/triangle/square) 모델 로드 (ultralytics)
  2) bbox 내부 HSV 로 색 확정 (v5: "색은 bbox 내부 HSV 확정")
  3) /camera/depth/image_raw 로 distance 채우기
  4) enable=false 전환 시 모델을 GPU 에서 내리는(또는 유지하는) 정책 결정
"""
import rclpy
...
class DockMarkDetector(Node):
    ...
    def on_image(self, msg: Image):
        ...
        for color, _cnt, area, cx in hsv_blobs(frame):
